...
## 1 **Grammatica a struttura di frase, come si definisce la grammatica?**
   1. La e definita tramite una quadrupla di G= {V, Sigma, P, S}  
   **V** è l'alfabeto finito detto vocabolario totale  
   **Sigma** e l'alfabeto dei simboli terminali contenuto in V           
   **S** e il simbolo iniziale o assioma  
   **P** è un insieme finito di espressioni della forma  
   2. con alfa che e detto insieme delle produzioni E le lettere N = V - Sigma si dicono Variabili e si indicano con le lettere maiuscole e le minuscole si dicono terminali.

## 2 **Chiusura di Kleene** 
   1. La chiusura di Kleene in un linguaggio L 
     L* = U n≥0 L^n = {u1u2 · · · un | n ≥ 0, u1, u2, . . . , un ∈ L}.  
    **Spiegato** La chiusura di kleene di un linguaggio e l'insieme di tutte le possibile concatenazioni dei simboli di un linguaggio.  
    avendo un alfabeto [a, b] la chiusura di kleen sarà  
    A0 = Epsilon   
    A1 = ab  
    A2 = aa, bb, ba, ab  
    A3 = A1 * A2
